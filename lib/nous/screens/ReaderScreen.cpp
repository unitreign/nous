...
void ReaderScreen::update(const ButtonState& buttons, DrawBuffer& buf, IRuntime& /*runtime*/) {
  ...
  const Button logical_next_front = inv_bottom ? Button::Button2 : Button::Button3;
  const Button logical_prev_front = inv_bottom ? Button::Button3 : Button::Button2;
  const Button logical_next_side  = inv_side   ? Button::Down    : Button::Up;
  const Button logical_prev_side  = inv_side   ? Button::Up      : Button::Down;
