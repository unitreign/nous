...
void ListMenuScreen::get_button_labels(const char* out[4]) const {
...
void ListMenuScreen::draw_lyra_tooltip_bar(DrawBuffer& buf, const BitmapFont& sf,
                                            int W, int box_y, const char* labels[4]) {
  static constexpr int kBotPad = 5;
  const int box_w  = W * 176 / 480;
  const int box_lx = W * 53 / 480;
  const int gap    = W * 22 / 480;
  const int box_rx = box_lx + box_w + gap;
  const int l_div  = box_lx + box_w / 2;
  const int r_div  = box_rx + box_w / 2;

  const int box_h  = kBotPad + sf.y_advance() + kBotPad;
  const int text_y = box_y + kBotPad + sf.baseline();

  auto draw_box = [&](int bx) {
    buf.fill_rect(bx,              box_y,             box_w, 1,     false);
    buf.fill_rect(bx,              box_y + box_h - 1, box_w, 1,     false);
    buf.fill_rect(bx,              box_y,             1,     box_h, false);
    buf.fill_rect(bx + box_w - 1,  box_y,             1,     box_h, false);
  };
  draw_box(box_lx);
  buf.fill_rect(l_div, box_y, 1, box_h, false);
  draw_box(box_rx);
  buf.fill_rect(r_div, box_y, 1, box_h, false);

  const int centers[4] = {box_lx + box_w / 4, box_lx + 3 * box_w / 4,
                           box_rx + box_w / 4, box_rx + 3 * box_w / 4};
  for (int i = 0; i < 4; ++i) {
